...
ENUNCIADO:
    "Utilizar o método dos retângulos para obter uma aproximação da área da região A.
    Cada função tem suas instruções especificadas no seu cabeçalho.
    "
...
def area_por_retangulos(f, a, b, k):
    ...
    while (n <= k):
        soma = ((b - a)/k) * f(a + (b-a)/(2 * k ) + (b-a) * m /k) + soma
        n = n + 1
        m = m + 1
...
def area_aproximada(f, a, b, eps=EPSILON):
    ...
    while (erro_rel(area, areaAntes) > eps):
        areaAntes = area
        n += 1
        k = math.pow(2, n)
        area = area_por_retangulos(f, a, b, math.pow(2, n))
        
    k = int(k)
    
    return k, area # para retornar um int e um float 
